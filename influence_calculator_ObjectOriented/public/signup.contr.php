...
class SignupContr extends Signup
{
    ...
    public function __construct($fullName, $username, $email, $password, $conpassword, $check, $files, $regNo)
    {
    ...
    public function signUser()
    {
        if ($this->emptyInput() == true) {
            ...
            header("Location: ../pages/registration/signup.php?error=emptyfields");
            exit();
        }
        if ($this->passwordCheck() == true) {
            $this->set_message("error", "Passwords do not match");
            header("Location: ../pages/registration/signup.php??error=passwordmatch");
            exit();
        }
        if ($this->userTaken() == false) {
            $this->set_message("error", "User already exists");
            header("Location: ../pages/registration/signup.php??error=userTaken");
            exit();
        }
        if ($this->invalidEmail() == false) {
            $this->set_message("error", "Invalid Email format");
            header("Location: ../pages/registration/signup.php??error=invalidEmail");
            exit();
        }

        if ($this->invalidUsername() == false) {
            $this->set_message("error", "Invalid format");
            header("Location: ../pages/registration/signup.php??error=invalidUsername");
            exit();
        }

        if ($this->passwordLength() == false) {
            $this->set_message("error", "Password should not be less that 8 characters");
            header("Location: ../pages/registration/signup.php??error=Passwordtooshort");
            exit();
        }
        ...
        $this->RegisterUser($this->fullName, $this->username, $this->email, $this->password, $this->newName(), $this->regNo);
    }
}
